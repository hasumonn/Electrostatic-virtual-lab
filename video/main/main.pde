...
void setup() {
 ...
 avatar = loadShape("avatar.svg");
 rectangle = createShape(RECT,0,0,100,80);
...
void onTickEvent(CountdownTimer t, long timeLeftUntilFinish){
  ...
  if (haply_board.data_available()) {
    ...
    current_charge.x_pos =  int(( pos_ee.x)*pixelsPerMeter + 400  );
    current_charge.y_pos = int((pos_ee.y )*pixelsPerMeter - 200);
